...
public class AdminPage extends JPanel {
    ...
    private JPanel buildAdminHero(){
        JPanel hero=new JPanel(new GridBagLayout()){
            @Override protected void paintComponent(Graphics g){
                ...
                g2.setPaint(new GradientPaint(0,0,ADMIN_STRIPE,getWidth(),getHeight(),new Color(50,50,120)));
    ...
    private JPanel buildProductSection(){
        ...
        JPanel row1=makeFieldRow(
                "Product ID",productIdField=makePlainField("e.g. PRD-001"),"Product Name",productNameField=makePlainField("e.g. Margherita Pizza"));
        card.add(row1);
        card.add(Box.createVerticalStrut(16));

        // Row 2 — Cost | Nutritional Value
        JPanel row2=makeFieldRow(
                "Cost ($)",productCostField=makePlainField("e.g. 25.00"),"Nutritional Value",productNutritionalField=makePlainField("e.g. 850 kcal"));
        card.add(row2);
        card.add(Box.createVerticalStrut(16));

        // Row 3 — Image path | Action dropdown
        JPanel row3=makeFieldRow(
                "Image Path / URL",productImgField=makePlainField("e.g. /images/pizza.png"),"Action",buildActionDropdownPanel());
        card.add(row3);
        ...
        productLog.setBorder(new CompoundBorder(
                new LineBorder(new Color(210,210,225),1),new EmptyBorder(8,10,8,10)));
    ...
    private JPanel buildActionDropdownPanel(){
        ...
        actionDropdown.setBorder(BorderFactory.createCompoundBorder(
                new LineBorder(new Color(210,200,195),1),new EmptyBorder(4,8,4,8)));
    ...
    private void handleProductSubmit(){
        ...
        switch(action){
            case "Add":
                if(!id.equals("")&&!name.equals("")&&!cost.equals("")&&!nutritVal.equals("")&&!img.equals("")){                   
    ...
    private JPanel makeFieldRow(String label1,JComponent field1,String label2,JComponent field2){
